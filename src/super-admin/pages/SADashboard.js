...
export default function SADashboard() {
  ...
  const load = useCallback(async () => {
    try {
      setError(false);
      setLoading(true); // ✅ ensure loading resets on retry
      ...
      // ✅ Handle token issues explicitly
    ...
    <SALayout>
      <div className="sa-dash-hero sa-animate-in">
        ...
        <p className="sa-dash-sub">
          Live snapshot across all parks. Auto-refreshes every 60s.
        </p>
      </div>
              ...
              {recentParks.length === 0 ? (
                ...
                recentParks.map((park) => (
                  <tr key={park.id}>
                    ...
                    <td style={{ fontFamily: "var(--sa-mono)" }}>
                      {park._count?.trips ?? 0}
                    </td>
                    <td style={{ fontFamily: "var(--sa-mono)" }}>
