...
/* ── Time formatter — Nigerian locale ────────────────────────── */
...
/* ═══════════════════════════════════════════════════════════════
   TRIP CARD
   CHANGED:
     - Flexible card now shows ticket creation time always
     - Flexible card shows departure time only when trip is FULL
       (i.e. departureTime was auto-set by the backend)
     - Scheduled card display is byte-for-byte identical to before
═══════════════════════════════════════════════════════════════ */
function TripCard({ trip, onDepart, onCancel, role }) {
  ...
  const isFlexible  = trip.tripType === "FLEXIBLE";

  // A flexible trip is considered "departed" or "full with time set"
  // when the backend has assigned a departureTime (auto-set when seats filled)
  const flexIsFull  = isFlexible && trip.status === "FULL" && trip.departureTime;
  ...
  return (
    <div className={`trip-card trip-card--${trip.tripType.toLowerCase()} ${trip.status !== "OPEN" ? "trip-card--inactive" : ""}`}>
      ...
      <div className="trip-card-badges">
        <span className={`pill ${isFlexible ? "pill-purple" : "pill-blue"}`}>
          {isFlexible ? "⚡ Flexible" : "🕐 Scheduled"}
        </span>
        <span className={`pill ${sc.cls}`}>{sc.label}</span>
        {trip.nearDeparture && !flexIsFull && (
          <span className="pill pill-amber">🔔 Ready to depart</span>
        )}
        {flexIsFull && (
          <span className="pill pill-amber">🚌 Departed</span>
        )}
      </div>
      ...
      {/* ── SCHEDULED: show fixed departure time (unchanged) ── */}
      {!isFlexible && trip.departureTime && (
      ...
      {/* ── FLEXIBLE: show creation time + departure time when full ── */}
      {isFlexible && (
        <div className="trip-card-flexible-times">
          {/* Ticket creation time — always shown */}
          <div className="trip-flex-time-row">
            <span className="trip-flex-time-label">🎟️ Created</span>
            <span className="trip-flex-time-value" style={{ fontFamily: "var(--mono)" }}>
              {fmtDateTime(trip.createdAt)}
            </span>
          </div>

          {/* Departure time — only shown once backend has set it (trip full) */}
          {flexIsFull ? (
            <div className="trip-flex-time-row trip-flex-time-row--departure">
              <span className="trip-flex-time-label">🚌 Departed</span>
              <span className="trip-flex-time-value trip-flex-time-value--departed" style={{ fontFamily: "var(--mono)" }}>
                {fmtDateTime(trip.departureTime)}
              </span>
            </div>
          ) : (
            <div className="trip-flex-time-row">
              <span className="trip-flex-time-label">🚌 Departure</span>
              <span className="trip-flex-time-value trip-flex-time-value--pending">
                Departs when full
              </span>
            </div>
          )}
      ...
      {/* Fill threshold note — flexible only */}
      {isFlexible && trip.fillThreshold && trip.fillThreshold < trip.totalSeats && (
      ...
      <div className="modal-box">
        ...
        <div className="form-group">
          ...
          <div className="trip-type-radios">
            <label className={`trip-radio-label ${isScheduled ? "trip-radio-label--active" : ""}`}>
              <input type="radio" name="tripType" value="SCHEDULED" checked={isScheduled} onChange={handle} className="trip-radio-input" />
            ...
            <label className={`trip-radio-label ${!isScheduled ? "trip-radio-label--active" : ""}`}>
              <input type="radio" name="tripType" value="FLEXIBLE" checked={!isScheduled} onChange={handle} className="trip-radio-input" />
              <span className="trip-radio-dot" />
              <span className="trip-radio-text">
        ...
        {isScheduled && (
          <div className="form-row">
            <div className="form-group">
              ...
              <input className="form-control" type="date" name="departureDate" value={form.departureDate} onChange={handle} />
            </div>
            <div className="form-group">
              <label className="form-label">Departure Time *</label>
              <input className="form-control" type="time" name="departureTime" value={form.departureTime} onChange={handle} />
            </div>
          </div>
        )}
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-control" name="price" type="number" placeholder="Uses route default" value={form.price} onChange={handle} />
          </div>
          <div className="form-group">
            <label className="form-label">Total Seats *</label>
            <input className="form-control" name="totalSeats" type="number" value={form.totalSeats} onChange={handle} />
          </div>
        </div>
        ...
        {!isScheduled && (
          <div className="form-group">
            ...
            <input className="form-control" name="fillThreshold" type="number" placeholder={`Default: all ${form.totalSeats || "?"} seats`} value={form.fillThreshold} onChange={handle} />
...
/* ═══════════════════════════════════════════════════════════════
   MAIN PAGE — unchanged except fetchAll includes createdAt
   (already returned by the backend, no backend change needed)
═══════════════════════════════════════════════════════════════ */
